...
final class InternalVertxTransactionContext implements VertxTransactionContext {
    private final Context context;
    private final AsyncTransactionContext delegate;

    InternalVertxTransactionContext(Context context, AsyncTransactionContext delegate) {
    ...
    public Future<VertxResultCursor> run(String query) {
        return fromStage(context, delegate.runAsync(query)).map(cursor -> new InternalVertxResultCursor(context, cursor));
    }

    @Override
    public Future<VertxResultCursor> run(String query, Value parameters) {
        return fromStage(context, delegate.runAsync(query, parameters))
                .map(cursor -> new InternalVertxResultCursor(context, cursor));
    }
